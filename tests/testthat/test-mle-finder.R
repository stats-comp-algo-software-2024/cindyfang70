test_that("linalg and optim least-sq coincide", {
  ...
  via_linalg_out <- hiper_glm(design, outcome, model = 'linear', option=list(mle_solver="pseudoinverse"))
...
test_that("newton and bfgs outputs coincide on logit model", {
  ...
  design <- data$design; outcome <- data$outcome
  via_newton_out <- hiper_glm(design, outcome, model = 'logit', options=list(mle_solver="newton-raphson"))
  via_bfgs_out <- hiper_glm(
    design, outcome, model = 'logit', options = list(mle_solver = 'BFGS')
  )
